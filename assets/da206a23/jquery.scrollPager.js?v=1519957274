(function ($) {
    ...
    var methods = {
        ...
        bind: function () {
            $wrapper.bind('scroll.' + name, scroll);
        },
        unbind: function () {
            $wrapper.unbind('.' + name);
        },
        ...
        retrieve: function () {
            if (!options.state.isPaused && 
!options.state.isLoadingNextPage) {
...
options.pjax.container);
                        },
                        success: function (text) {
                            options.state.tempHTML = 
$('<div>').append(text);
...
$(options.wrapper).find(options.pagination).html(options.state.tempHTML.find(options.pagination).html());
                            ...
                            // Append items
                            var items = 
options.state.tempHTML.find(options.item);
...
object.append($('<div/>').addClass(options.loadingTextWrapperClass).html(options.loadingText));
        ...
        hideLoadingText: function () {
            var target = object.find('.' + 
options.loadingTextWrapperClass);
            if (target.length) {
